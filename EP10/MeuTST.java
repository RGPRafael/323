...
public class MeuTST<Value extends Comparable<Value>> {
    ...
    public Iterable<String> keysWithPrefixByValue(String prefix) {
        ...
        RedBlackBST<Value, String>st = new RedBlackBST<Value, String>();
        
        for (String s2 : keysWithPrefix(prefix))
            st.put(get(s2), s2 );

        Stack<String> stack = new Stack<String>();
        
        for (Value s1 : st.keys())
            stack.push(st.get(s1));
    ...
    */
    ...
    private Node<Value> delete(Node<Value> x, String k, int d) {
        if (x == null) return null;
        if (d == k.length() - 1){
            if(x.val != null) n--;
            x.val = null;
            ...
            char c = k.charAt (d);
            if (c < x.c)        x.left  = delete(x.left,  k, d);
            else if (c > x.c)   x.right = delete(x.right, k, d);
            else                x.mid   = delete(x.mid,   k, d+1);
    ...
    public void delete(String key) {
        // TAREFA
        if (key == null) throw new NullPointerException("delete(): argument key is null");
    ...
    public static void main(String[] args) {
        ...
        MeuTST<Long> auto = new MeuTST<Long>();
        try {
        ...
        StdOut.println("TESTE keysWithPrefixByValue e DELETE.....DIGITE: ");
        while (!StdIn.isEmpty()) {
            String item = StdIn.readString();
            StdOut.println("keysWithPrefix("+ item + "):");
            for (String s : auto.keysWithPrefix(item))
                StdOut.println(s + " : " + auto.get(s));

            StdOut.println("--------------------------------------------");
            
            StdOut.println("keysWithPrefixByValue("+ item + "):");
            for (String s : auto.keysWithPrefixByValue(item))
                StdOut.println(s + " : " + auto.get(s));

            StdOut.println("--------------------------------------------");

            StdOut.println("delete(" + item + ")");
            auto.delete(item);
            StdOut.println("keysWithPrefixByValue("+ item + ") APOS DELETE:");
            for (String s : auto.keysWithPrefixByValue(item))
                StdOut.println(s + " : " + auto.get(s));
