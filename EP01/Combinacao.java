/****************************************************************
    Nome: Rafael Gonlçalves Pereira da Silva.
    NUSP: 9009600.
...
public class Combinacao {
    ...
    public static void combinacao(int n, int k) {
      int[] array = new int[n];
      int[] comb = new int[k];
      for(int i = 0; i < n; i++)array[i] = i+1; // gera um vetor com n inteiros
      for(int i = 0; i < k; i++)comb[i] = 0;    //gera um vetor de tamanho k para combinações.
      printcomb(array, comb, 0, n-1, 0, k);
    }
    public static void printcomb(int array[], int comb[], int L, int R, int F, int k){
      if( F == k){
      	if(opcao != 1){
        	for(int j = 0; j < k; j++)StdOut.print(comb[j]);
        	StdOut.println();
    	}
        count++;
      }
      for(int i = L; i <= R && F < k; i++){
        comb[F] = array[i];
        printcomb(array, comb, i+1, R, F + 1, k);
      }
